...
Dockerfile.3.5: Dockerfile
	sed -e 's,FROM camptocamp/python-gis:3\.6,FROM camptocamp/python-gis:3.5,' < $< > $@

build_docker3.5: Dockerfile.3.5
	docker build -t $(DOCKER_BASE):latest -f Dockerfile.3.5 .

clean:
	rm -rf dist c2cwsgiutils.egg-info Dockerfile.3.5 .venv .mypy_cache
