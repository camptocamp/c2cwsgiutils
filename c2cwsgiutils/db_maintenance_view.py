...
def install_subscriber(config: pyramid.config.Configurator) -> None:
    ...
    if auth.is_enabled(config, ENV_KEY, CONFIG_KEY):
        config.add_route("c2c_db_maintenance", _utils.get_base_path(config) + r"/db/maintenance",
                         request_method="GET")
        config.add_view(_db_maintenance, route_name="c2c_db_maintenance", renderer="fast_json",
                        http_cache=0)
        _restore(config)
